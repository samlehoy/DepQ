...
function Quran() {
  ...
  return (
    <div className="px-4 py-6 sm:px-6 md:px-10 w-full max-w-5xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-xl sm:text-display-lg text-[var(--ds-primary)] tracking-tight mb-1 sm:mb-2">{t.quran_title}</h1>
      ...
      <div className="mb-6 sm:mb-8 relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-3 sm:py-4 rounded-xl bg-[var(--ds-surface-container-lowest)] border border-[var(--ds-outline-variant)]/40 focus:outline-none focus:ring-2 focus:ring-[var(--ds-primary-fixed-dim)] focus:border-transparent shadow-[var(--shadow-soft)] text-[var(--ds-on-surface)] placeholder:text-[var(--ds-outline)] text-sm sm:text-base"
        />
      </div>
      ...
      <div className="flex flex-col space-y-3">
        ...
        {filtered.map(surah => (
          <Link
            key={surah.id}
            to={`/quran/${surah.id}`}
            className="group bg-[var(--ds-surface-container-lowest)] p-3 sm:p-4 rounded-xl shadow-[var(--shadow-soft)] border border-[var(--ds-primary)]/5 flex items-center justify-between hover:border-[var(--ds-primary)]/20 hover:shadow-[var(--shadow-card)] transition-all duration-200 gap-2"
          >
            <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
              ...
              <div>
                <h3 className="font-semibold text-[var(--ds-on-surface)] text-base sm:text-lg group-hover:text-[var(--ds-primary)] transition-colors truncate">
                  {surah.name_simple}
                </h3>
